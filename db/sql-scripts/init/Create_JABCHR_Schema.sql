...
CREATE TABLE IF NOT EXISTS `jabc_db`.`ONBOARDING_TASK` (
  `ONBOARDING_TASK` INT NOT NULL AUTO_INCREMENT,
  `EMPLOYEE_ID` INT NOT NULL,
  `CREATED_DATE` DATE NOT NULL,
  `DUE_DATE` DATE NULL,
  `EXPIRY_DATE` BIGINT NULL,
  `DESCRIPTION` VARCHAR(512) NOT NULL,
  `DOCK_TYPE_ID` INT NULL,
  `ACTUAL_FILE` BLOB NULL COMMENT 'Path to this support document',
  `REQUIRE_DOC` TINYINT(1) NOT NULL COMMENT '0 = Just a reminder, 1 = Requires the upload of a document',
  PRIMARY KEY (`ONBOARDING_TASK`),
  INDEX `TYPE_ID_idx` (`DOCK_TYPE_ID` ASC),
  INDEX `FK_SUPPORT_DOC_EMPLOYEE_idx` (`EMPLOYEE_ID` ASC),
  CONSTRAINT `FK_TYPE_ID`
    FOREIGN KEY (`DOCK_TYPE_ID`)
