...
BEGIN
    ...
    SELECT idHR_RECORD INTO checker FROM HR_RECORD where HR_RECORD.EMAIL = EMAIL AND HR_RECORD.STATUS != 0;
    IF checker = 0 THEN
        SIGNAL SQLSTATE '45000'
        SET MESSAGE_TEXT = 'The email is not registered.';
    END IF;

    SELECT idHR_RECORD INTO checker2 FROM HR_RECORD WHERE HR_RECORD.EMAIL = EMAIL AND HR_RECORD.PASSWORD = PASSWORD AND HR_RECORD.STATUS != 0 LIMIT 1;
    IF checker2 = 0 THEN
        SIGNAL SQLSTATE '45000'
        SET MESSAGE_TEXT = 'The password is incorrect';
    END IF;

    SELECT * FROM HR_RECORD WHERE idHR_RECORD = checker2 LIMIT 1;
END$$
DELIMITER ;